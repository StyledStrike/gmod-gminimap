...
--hook.Add( "InitPostEntity", "GMinimap.Init", function()
    for _, v in pairs( list.Get( "NPC" ) ) do
        local kv = v.KeyValues

        if kv and ( kv.SquadName or kv.squadname ) then
            npcSquads[v.Class] = kv.SquadName or kv.squadname
        end
    end
--end )
...
local SQUAD_COLORS = {
    ...
    ["overwatch"] = Color( 255, 100, 100 ),
    ["novaprospekt"] = Color( 255, 100, 100 ),
    ["poison"] = Color( 255, 150, 100 ),
    ["zombies"] = Color( 255, 150, 100 ),
    ["resistance"] = Color( 100, 100, 255 ),
}
...
local function UpdateNPCBlip( ent, alpha )
    ...
    local squad = npcSquads[ent:GetClass()]
    local color
